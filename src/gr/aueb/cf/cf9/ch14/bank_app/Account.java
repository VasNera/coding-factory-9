...
public class Account {
     ...
     * @throws SsnNotValidException          if SSN does not match
     */
    public void withdraw(double amount, String ssn)throws NegativeAmountException, InsufficientBalanceException,SsnNotValidException{
        try{
            if (amount < 0)throw new NegativeAmountException("The amount" + amount + "must be positive.");
            if (amount > balance) throw new InsufficientBalanceException("The balance= " + balance + " is insufficient.");
            if (!isSsnValid(ssn))throw new SsnNotValidException("The ssn " + ssn +"does not match.");            //καλεί τη μέθοδο από κάτω.
            balance -= amount;
